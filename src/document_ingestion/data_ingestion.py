...
class FaissManager:
        ...
        try:
            ...
            if self.meta_path.exists():
                try:
                    ...
                    self.log.warning(f"Failed to load the metadata, starting fresh", error = str(e))
                    self._meta = {"rows":{}}
    ...
    def _save_metadata(self):
        try:
            return self.meta_path.write_text(json.dumps(self._meta, indent=2), encoding="utf-8")
        except Exception as e:
    ...
    def load_or_create(self):
        try:
            if self._exists():
                self.log.info("Loading the Faiss index")
                self.vs = FAISS.load_local(str(self.index_dir), self.emb(),
                                        allow_dangerous_deserialization=True)
                self.log.info("Loaded the Faiss index successfully")
                return self.vs
